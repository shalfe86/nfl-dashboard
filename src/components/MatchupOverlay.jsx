...
export const MatchupOverlay = ({ game, leagueStats, predictions, onClose }) => {
  if (!game) return null;
  ...
  // Get AI Prediction from DB
  const pred = predictions ? predictions[game.game_id] : null;
  ...
  // Use DB prediction if available, otherwise estimate
  const finalHome = pred ? pred.home_proj_score : Math.max(0, Math.round(20 + ((homeStats.qb - 50) / 5) + homeAdvantage));
  const finalAway = pred ? pred.away_proj_score : Math.max(0, Math.round(20 + ((awayStats.qb - 50) / 5)));
  const winProb = pred ? pred.win_prob : 50 + Math.abs(spread) * 3;
  const winner = finalHome > finalAway ? teamId : opponent;
          ...
          <div className="flex flex-col gap-6">
             <div className="bg-gradient-to-br from-slate-900 to-slate-950 rounded-2xl p-8 border border-white/10 flex flex-col items-center text-center shadow-xl relative overflow-hidden">
                ...
                <div className="flex items-end gap-2 mb-2">
                    <span className="text-5xl font-black text-white">{winner}</span>
                    <span className="text-lg text-slate-400 font-bold mb-2">to Win</span>
                </div>
                <div className="text-xs text-slate-500 font-mono mb-8">Confidence: <span className="text-white">{Math.min(99, winProb).toFixed(0)}%</span></div>
